...
@app.route('/login/')
def login():
	""" Creates a state token, (32) characters long, to prevent request forgery."""
...
def createUser(login_session):
	if(login_session['email']=="jye0325@gmail.com"):
		newUser = User(name=login_session['username'], email=login_session[
			'email'], picture=login_session['picture'], status='admin')
	else:
   		newUser = User(name=login_session['username'], email=login_session[
...
def newCategory():
	...
	if user.status != 'admin':
		return "<script>function myFunction() {alert('Sorry only admin privileges are allowed to add new categories for now.');location.href='http://localhost:5000';}</script><body onload='myFunction()'>"
...
def editCategory(category_id):
	...
	if category.user_id == user.id or user.status == 'admin':
		...
		return "<script>function myFunction() {alert('Sorry only the creator of this category, or an admin, can edit this.');location.href='http://localhost:5000';}</script><body onload='myFunction()'>"
	if request.method == 'POST':
...
def deleteCategory(category_id):
	...
	if category.user_id == user.id or user.status == 'admin':
		...
		return "<script>function myFunction() {alert('Sorry only the creator of this category, or an admin, can delete this.');location.href='http://localhost:5000';}</script><body onload='myFunction()'>"
	if request.method == 'POST':
...
def editItem(category_id, item_id):
	...
	if item.user_id == user.id or user.status == 'admin':
		...
		return "<script>function myFunction() {alert('Sorry only the creator of this item, or an admin, can edit this.');location.href='http://localhost:5000';}</script><body onload='myFunction()'>"
	if request.method == 'POST':
...
def deleteItem(category_id, item_id):
	...
	if item.user_id == user.id or user.status == 'admin':
		...
		return "<script>function myFunction() {alert('Sorry only the creator of this item, or an admin, can delete this.');location.href='http://localhost:5000';}</script><body onload='myFunction()'>"
	if request.method == 'POST':
		session.delete(item)
		session.commit()
